...
# Omogočimo dodajanje novega izdelka v podatkovno bazo.
def add_product():
    if not login_required(): return redirect(url_for('login'))
    if request.method=='POST':
        f=request.form
        db=get_db()
        db.execute('''INSERT INTO products(product_id,product_name,brand_name,price_usd,rating,reviews,size,primary_category,secondary_category,tertiary_category,ingredients,highlights,new_product,online_only,out_of_stock,sephora_exclusive,limited_edition) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(
            f.get('product_id') or None,f.get('product_name','').strip(),f.get('brand_name','').strip(),float(f.get('price_usd') or 0),float(f.get('rating') or 0),int(f.get('reviews') or 0),f.get('size',''),f.get('primary_category',''),f.get('secondary_category',''),f.get('tertiary_category',''),f.get('ingredients',''),f.get('highlights',''),parse_bool(f.get('new_product')),parse_bool(f.get('online_only')),parse_bool(f.get('out_of_stock')),parse_bool(f.get('sephora_exclusive')),parse_bool(f.get('limited_edition'))))
        db.commit(); flash('Product added.','success'); return redirect(url_for('products'))
    return render_template('product_form.html', mode='Add')

@app.route('/edit/<int:pid>', methods=['GET','POST'])
# Omogočimo urejanje podatkov obstoječega izdelka.
def edit_product(pid):
    ...
    if request.method=='POST':
        f=request.form
        db.execute('''UPDATE products SET product_name=?,brand_name=?,price_usd=?,rating=?,reviews=?,size=?,primary_category=?,secondary_category=?,tertiary_category=?,ingredients=?,highlights=?,new_product=?,online_only=?,out_of_stock=?,sephora_exclusive=?,limited_edition=? WHERE id=?''',(
            f.get('product_name','').strip(),f.get('brand_name','').strip(),float(f.get('price_usd') or 0),float(f.get('rating') or 0),int(f.get('reviews') or 0),f.get('size',''),f.get('primary_category',''),f.get('secondary_category',''),f.get('tertiary_category',''),f.get('ingredients',''),f.get('highlights',''),parse_bool(f.get('new_product')),parse_bool(f.get('online_only')),parse_bool(f.get('out_of_stock')),parse_bool(f.get('sephora_exclusive')),parse_bool(f.get('limited_edition')),pid)); db.commit(); flash('Product updated.','success'); return redirect(url_for('product',pid=pid))
